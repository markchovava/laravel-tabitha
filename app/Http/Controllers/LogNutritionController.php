...
use App\Http\Resources\LogNutritionResource;
...
class LogNutritionController extends Controller
{
    ...
    public function indexByMonthlyUser($userId, $monthlyReportId){
        if(!empty($userId) && !empty($monthlyReportId)) {
            $data = LogNutrition::where('userId', $userId)
                ->where('monthlyReportId', $monthlyReportId)
                ->paginate(20);
            return LogNutritionResource::collection($data);
        }
    ...
    public function store(Request $request){
        ...
        $data->save();
        return response()->json([
            'data' => new LogNutritionResource($data),
    ...
    public function view($id){
        $data = LogNutrition::with(['user'])
            ->find($id);
        return new LogNutritionResource($data);
    }
