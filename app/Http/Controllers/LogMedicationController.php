...
use App\Http\Resources\LogMedicationResource;
use App\Models\LogMedication;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class LogMedicationController extends Controller
{
    ...
    public function indexByMonthlyUser($userId, $monthlyReportId){
        if(!empty($userId) && !empty($monthlyReportId)) {
            $data = LogMedication::where('userId', $userId)
                ->where('monthlyReportId', $monthlyReportId)
                ->paginate(20);
            return LogMedicationResource::collection($data);
        }
    ...
    public function store(Request $request){
        ...
        $data->save();
        return response()->json([
            'data' => new LogMedicationResource($data),
    ...
    public function view($id){
        $data = LogMedication::with(['user'])
            ->find($id);
        return new LogMedicationResource($data);
    }
