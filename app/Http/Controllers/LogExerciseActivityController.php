...
use App\Http\Resources\LogExerciseActivityResource;
use App\Models\LogExerciseActivity;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class LogExerciseActivityController extends Controller
{
    
    public $upload_location = 'assets/img/log/';

    public function indexAll(){
        $data = LogExerciseActivity::with(['user'])
                ->orderBy('updated_at', 'DESC')
                ->get();
        return LogExerciseActivityResource::collection($data);
    }

    public function indexByMonthlyUser($userId, $monthlyReportId){
        if(!empty($userId) && !empty($monthlyReportId)) {
            $data = LogExerciseActivity::where('userId', $userId)
    ...
    public function store(Request $request){
        ...
        $data->save();
        return response()->json([
            'data' => new LogExerciseActivityResource($data),
    ...
    public function update(Request $request, $id){
        $userId = Auth::user()->id ?? null;
        $data = LogExerciseActivity::find($id);
        $data->userId = $userId ?? $request->userId;
        $data->details = $request->details;
        $data->patient = $request->patient;
        $data->assistant = $request->assistant;
    ...
    public function view($id){
        $data = LogExerciseActivity::with(['user'])
            ->find($id);
        return new LogExerciseActivityResource($data);
    }
